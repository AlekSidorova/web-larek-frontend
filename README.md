...
- private api: AppApi — Объект API для выполнения запросов.

Методы:
- sync fetchCards(): Promise<void> — Асинхронно загружает карточки товаров с сервера, обновляет внутренний массив cards и эмитирует событие cards:update при успешной загрузке. В случае ошибки эмитирует событие cards:error.
...
Методы:
- private initFieldListeners(): void — Устанавливает обработчики событий для полей ввода.
- phoneInput — Форматирует номер телефона при вводе и эмитирует событие order:fieldChange с обновлённым номером.
- emailInput — Эмитирует событие order:fieldChange с введённым адресом электронной почты при изменении.
- private validate(): void — Выполняет валидацию формы, проверяя правильность введённых номера телефона и электронной почты. Устанавливает состояние кнопки отправки в зависимости от валидности данных.
- private formatPhone(value: string): string — Форматирует номер телефона в заданный формат (например, "+7 (000) 000 00 00"). Удаляет все нецифровые символы и обрезает до 10 цифр.
- protected handleSubmit(): void — Обрабатывает событие отправки формы. Если кнопка отправки не заблокирована, эмитирует событие checkout:step2Completed с информацией о введённых адресе электронной почты и номере телефона.
- protected onFieldChange(field: keyof IOrderForm, value: string): void —  Переопределяет метод из базового класса для эмита события изменения поля и запускает валидацию.
