...
export class AppApi extends Api {
  ...
  constructor(baseUrl: string, cdn: string, options?: RequestInit) {
    super(baseUrl, options); //базовый url и опциональные настройки для запросов
    this.cdn = cdn;
  }
  ...
  getProductList(): Promise<ICard[]> {
    return this.get('/product/').then((data: ApiListResponse<ICard>) => //then - когда сервер вернул данные
      data.items.map((item) => ({ //map - создаем новый объект для каждой карточки
  ...
  createOrder(order: IOrder) {
    return this.post('/order', order).then(
      (data: IOrderResult) => data //обработчик, когда сервер возвращает результат
    );
  }
}